...
import 'package:common_utils/utils/time_utils.dart';
...
class TimelineUtil {
  ...
  static String _getYesterday(int timeMillis, TimelineInfo info, DayFormat dayFormat) {
    return info.customYesterday() + (dayFormat == DayFormat.Full ? (" ${TimeUtils.getTime(milliseconds: timeMillis, format: DateFormat.HOUR_MINUTE) as String}") : "");
  }
  ...
  static String? _getYear(int timeMillis, DayFormat dayFormat) {
    if (dayFormat != DayFormat.Simple) {
      return TimeUtils.getTime(milliseconds: timeMillis, format: (dayFormat == DayFormat.Common ? DateFormat.YEAR_MONTH_DAY : DateFormat.YEAR_MONTH_DAY_HOUR_MINUTE));
    }
    return "";
  }
  ...
  static String? _formatDays(int timeMillis, num days, TimelineInfo timelineInfo, DayFormat dayFormat) {
    ...
    switch (dayFormat) {
      ...
      case DayFormat.Common:
        timeline = TimeUtils.getTime(milliseconds: timeMillis, format: DateFormat.MONTH_DAY);
        break;
      case DayFormat.Full:
        timeline = TimeUtils.getTime(milliseconds: timeMillis, format: DateFormat.MONTH_DAY_HOUR_MINUTE);
        break;
    }
    return timeline;
  }
